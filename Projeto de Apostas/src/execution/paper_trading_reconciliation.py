...
class PaperTradingReconciler:
    ...
    def print_report(self, days: Optional[int] = None) -> None:
        ...
        print("\n" + "=" * 70)
        print("  Paper Trading Reconciliation Report")
        print("=" * 70)
        print(f"  Period:       {r.period_start} to {r.period_end}")
        print(f"  Total trades: {r.total_trades} ({r.settled_trades} settled)")
        print(f"  Simulated P&L: {r.total_simulated_pnl:+.2f}")
        print(f"  Actual P&L:    {r.total_actual_pnl:+.2f}")
        print(f"  Net error:     {r.net_pnl_error:+.2f}")
        print(f"  Correlation:   {r.correlation:.3f}")
        print(f"  RMSE:          {r.rmse:.2f}")
        print(f"  MAPE:          {r.mape:.1f}%")
        print(f"  Status:        {'PASS' if r.is_accurate else 'REVIEW NEEDED'}")
        print("=" * 70 + "\n")
